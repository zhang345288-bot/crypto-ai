...
if is_valid_key:
    try:
        ...
        # 使用最新的 Gemini 2.5 Flash 模型，限制輸出token以節省配額
...
def generate_ai_analysis(
    ...
    # 縮短 rationale，只取前2個關鍵點以節省token
    short_rationale = rationale[:2]
    
    prompt = f"""加密貨幣分析師建議：

價格: ${last_price:.2f}, 趨勢: {trend}, 風險: {risk}
指標: RSI {indicators.get('rsi14', 'N/A')}, MACD {indicators.get('macd', 'N/A')}
評估: {'; '.join(short_rationale)}
建議: {action}

請簡潔回答3點: 市場分析、進出場策略、風險管理。用繁體中文。"""
